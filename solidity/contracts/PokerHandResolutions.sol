...
contract PokerHandResolutions {
  ...
  /**
	...
	*/
    ...
    function resolveWinner(address dataAddr) public {
		...
		*/
        ...
        if (dataStorage.allPlayersAtPhase(15) || dataStorage.allPlayersAtPhase(16)){
			...
			address verifiedWinner = address(0);
			uint resolutions=0;
			for (uint count = 1; count<dataStorage.num_Players(); count++) {
				if (dataStorage.declaredWinner(dataStorage.players(count)) == verifiedWinner) {
					resolutions++;
				} else {
					if (verifiedWinner == address(0)) {
						resolutions++;
			...
			if (resolutions != dataStorage.num_Players()) {
				...
				for (uint8 count=0; count < dataStorage.num_Players(); count++) {
     ...
     */
    function challenge (address dataAddr, address validatorAddr, uint256[] memory encKeys, uint256[] memory decKeys, uint256 challengeValue) payable public {
		PokerHandData dataStorage = PokerHandData(dataAddr);
        ...
        if (dataStorage.phases(msg.sender) != 19) {
            ...
            if (dataStorage.challenger() < address(2)) {
                //set just once
     ...
     */
    function L1Validate(address dataAddr, uint256[] memory encKeys, uint256[] memory decKeys, uint[] memory bestCards) public {
		PokerHandData dataStorage = PokerHandData(dataAddr);
